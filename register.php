...
<main id="main-content" class="auth-shell">
    ...
    <section class="form-card" aria-labelledby="register-title"><h2 id="register-title">Create account</h2><p>Already registered? <a href="<?= e(url('login.php')) ?>">Log in</a>.</p>
        ...
        <form method="post" action="<?= e(url('register.php')) ?>" data-validate novalidate>
            <?= csrf_field() ?>
            <div class="field"><label for="name">Full name</label><input id="name" name="name" type="text" value="<?= old('name') ?>" autocomplete="name" minlength="2" maxlength="80" required aria-describedby="name-error" <?= isset($errors['name']) ? 'aria-invalid="true"' : '' ?>><?php if (isset($errors['name'])): ?><span class="field-error" id="name-error"><?= e($errors['name']) ?></span><?php endif; ?></div>
            <div class="field"><label for="email">Email address</label><input id="email" name="email" type="email" value="<?= old('email') ?>" autocomplete="email" maxlength="190" required aria-describedby="email-error" <?= isset($errors['email']) ? 'aria-invalid="true"' : '' ?>><?php if (isset($errors['email'])): ?><span class="field-error" id="email-error"><?= e($errors['email']) ?></span><?php endif; ?></div>
            <div class="field"><label for="password">Password</label><input id="password" name="password" type="password" autocomplete="new-password" minlength="10" required aria-describedby="password-help password-error" <?= isset($errors['password']) ? 'aria-invalid="true"' : '' ?>><span class="field-help" id="password-help">10+ characters with uppercase, lowercase and a number.</span><?php if (isset($errors['password'])): ?><span class="field-error" id="password-error"><?= e($errors['password']) ?></span><?php endif; ?></div>
            <div class="field"><label for="password_confirm">Confirm password</label><input id="password_confirm" name="password_confirm" type="password" autocomplete="new-password" required aria-describedby="password-confirm-error" <?= isset($errors['password_confirm']) ? 'aria-invalid="true"' : '' ?>><?php if (isset($errors['password_confirm'])): ?><span class="field-error" id="password-confirm-error"><?= e($errors['password_confirm']) ?></span><?php endif; ?></div>
            <label class="check-row" for="privacy"><input id="privacy" type="checkbox" name="privacy" value="1" required aria-describedby="privacy-error" <?= isset($errors['privacy']) ? 'aria-invalid="true"' : '' ?> <?= isset($_POST['privacy']) ? 'checked' : '' ?>><span>I have read the <a href="<?= e(url('privacy.php')) ?>" target="_blank" rel="noopener">privacy notice</a>.</span></label><?php if (isset($errors['privacy'])): ?><span class="field-error" id="privacy-error"><?= e($errors['privacy']) ?></span><?php endif; ?>
